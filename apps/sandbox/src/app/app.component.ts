...
export class AppComponent implements OnInit, OnDestroy {
  ...
  globalController: TransformStreamDefaultController<VideoFrame> | undefined;
  timestamp: number | undefined;
  canvasElement2 = new OffscreenCanvas(1280, 720);
  canvasCtx2 = this.canvasElement2.getContext('2d');
  ...
  async ngOnInit(): Promise<void> {
    ...
    const feed = await this.mediaDevices.getUserMedia({
      video: {
        aspectRatio: {
          ideal: 16 / 9,
        },
    ...
    const [track] = feed.getVideoTracks();
    ...
    this.stream = new MediaStream([trackGenerator]);
  }
